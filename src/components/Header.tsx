...
// ATUALIZAÇÃO (Fase Frontend): Adicionada lógica para esconder "Explorar" e "Lives" de usuários gratuitos.
// ATUALIZAÇÃO (Debug 5): Corrigidos os caminhos de importação mistos (relativos e alias).

import React, { useState } from 'react';
import { NavLink, useNavigate } from 'react-router-dom';
// --- CORREÇÃO DE CAMINHO (Usando os caminhos mistos corretos) ---
import { Button } from './ui/button';
import { useAuth } from '../contexts/AuthProvider'; 
import newLogo from '../assets/logo_sem_fundo_limpo.png';
import api from '../services/api'; 
import PimentaShopModal from './PimentaShopModal';
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"; // Este @/ estava correto
// --- FIM DA CORREÇÃO ---
...
          _     <NavLink to="/cadastrar"><Button>Cadastrar</Button></NavLink>
              </div>
            )}
            {isLoggedIn && (
...
                  className="w-full justify-center py-2 text-base bg-pink-600 text-white h-auto mb-2"
            >
t             Comprar Pimentas 🌶️
              </Button>
      _       <NavLink to="/home" onClick={() => setIsMobileMenuOpen(false)} className={({ isActive }) => `block text-center py-2 text-base ${isActive ? 'text-primary' : 'text-white'}`}>Home</NavLink>
...
              
                {/* --- ★★★ ATUALIZAÇÃO (Fase Frontend) ★★★ --- */}
                {/* Só mostra "Explorar" e "Lives" se o plano NÃO for gratuito */}
section        {user?.tipo_plano !== 'gratuito' && (
                    <>
